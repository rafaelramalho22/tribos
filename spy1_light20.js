...
var scriptConfig = {
    ...
    translations: {
        ...
        pt_BR: {
            'Barbs Finder': 'Procurador de Bárbaras',
            'Min Points:': 'Pontos mánimos:',
            'Max Points:': 'Pontos míximos:',
            'Radius:': 'Campo:',
            'Barbs found:': 'Bárbaras encontradas:',
            'Coordinates:': 'Coordenadas:',
            'Error while fetching "village.txt"!':
...
(async function () {
        ...
        const content = `
                <div class="ra-grid ra-grid-4">
                    ...
                    <div class="ra-mb15">
                        ...
                        <input type="text" id="minPoints" value="200" class="ra-input">
                    </div>
                    <div class="ra-mb15">
                        <label for="maxPoints" class="ra-label">${twSDK.tt(
                            'Max Points:'
                        )}</label>
                        <input type="text" id="maxPoints" value="1100" class="ra-input">
                    </div>
                    <div class="ra-mb15">
                        <label for="ra_village_select">Aldeia:</label>
                        <select id="ra_village_select" class="ra-input"></select>
                    </div>
                    <div class="ra-mb15">
                        <label for="ra_spy">Batedor:</label>
                        <input type="number" id="ra_spy" value="1" min="0" class="ra-input"/>
                    </div>
                    <div class="ra-mb15">
                        <label for="ra_light">Cavalo Leve:</label>
                        <input type="number" id="ra_light" value="20" min="0" class="ra-input"/>
                    </div>
                    <div class="ra-mb15">
                        <label for="ra_heavy">Cavalo Pesado:</label>
                    ...
                    <div class="ra-mb15">
                        <label for="ra_sword">Espada:</label>
                        <input type="number" id="ra_sword" value="0" min="0" class="ra-input"/>
                    </div>
                </div>
                <div class="ra-mb15">
    ...
    function generateTableRows(barbs) {
        ...
        barbs.forEach((barb, index) => {
            ...
            const spy = Math.max(0, parseInt(document.getElementById('ra_spy').value) || 0);
    ...
    function populateMyVillagesSelect(villages) {
        const select = document.getElementById('ra_village_select');
        select.innerHTML = '';

        const myPlayerId = game_data.player.id;

        // usa a função do próprio script
        const myCoords = twSDK.filterVillagesByPlayerIds([myPlayerId], villages);

        myCoords.forEach(coord => {
            const [x, y] = coord.split('|');

            // encontrar dados completos da aldeia
            const villageData = villages.find(v => v[2] == x && v[3] == y);

            if (villageData) {
                const villageId = villageData[0];
                const villageName = twSDK.cleanString(villageData[1]);
                const points = villageData[5];

                const option = document.createElement('option');
                option.value = villageId;
                option.textContent = `${villageName} (${coord}) - ${points}`;

                // selecionar a atual por defeito
                if (villageId == game_data.village.id) {
    ...
    populateMyVillagesSelect(villages);
})();
